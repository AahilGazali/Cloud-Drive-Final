...
const StoragePlansPage = () => {
  ...
  const handlePlanSelect = (plan) => {
    if (plan.id === currentPlan.id) {
      window.alert(`You are already on the ${plan.name} plan!`);
      return;
    }
  ...
  const handlePaymentSubmit = (e) => {
    ...
    // Simulate payment processing
    const processing = window.confirm(`Confirm payment of ₹${selectedPlan.priceINR}/month for ${selectedPlan.name} plan (${selectedPlan.storage} GB)?`);
    ...
    if (processing) {
      ...
      setTimeout(() => {
        ...
        window.alert(`Payment successful! Your ${selectedPlan.name} plan with ${selectedPlan.storage} GB storage is now active.`);
